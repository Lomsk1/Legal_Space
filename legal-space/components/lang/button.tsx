...
function LangButton({ lang }: { lang: string }) {
  ...
  const handleClick = (e: MouseEvent<HTMLElement>) => {
    ...
    Cookie.set("lang", !language ? "eng" : "geo");
    location.reload();
  };
  ...
  return (
    <label
      ...
      className="relative inline-flex items-center cursor-pointer h-[40px] w-[85px] rounded-full bg-main-green-dark"
    >
      <input type="checkbox" className="sr-only peer " />

      <div
        className={`absolute bg-black rounded-full w-7 h-7 text-center pointer-events-none ${
